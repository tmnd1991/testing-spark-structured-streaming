...
abstract class QueryTest extends SparkFunSuite with PlanTest {
  ...
  // Timezone is fixed to UTC for those timezone sensitive tests (timestamp_*)
...
object QueryTest {
  ...
  def checkAnswer(df: DataFrame, expectedAnswer: Seq[Row], checkToRDD: Boolean = true): Option[String] = {
    ...
    scala.util.Try(df.collect().toSeq).map { sparkAnswer =>
  ...
  private def genError(expectedAnswer: Seq[Row], sparkAnswer: Seq[Row], isSorted: Boolean): String = {
  ...
  def includesRows(expectedRows: Seq[Row], sparkAnswer: Seq[Row]): Option[String] =
    if (!prepareAnswer(expectedRows, true).toSet.subsetOf(prepareAnswer(sparkAnswer, true).toSet)) {
      Some(genError(expectedRows, sparkAnswer, true))
    } else {
      None
    }
