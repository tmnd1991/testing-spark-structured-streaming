...
import org.apache.spark.sql.execution.streaming.continuous.{
  ContinuousExecution,
  EpochCoordinatorRef,
...
trait StreamTest extends QueryTest with TimeLimits with BeforeAndAfterAll {
   ...
   */
  object MultiAddData {
    def apply[A](source1: MemoryStream[A], data1: A*)(source2: MemoryStream[A], data2: A*): StreamAction = {
  ...
  object CheckAnswer {
    def apply[A: Encoder](data: A*): CheckAnswerRows = {
      ...
      CheckAnswerRows(data.map(d => toExternalRow.fromRow(encoder.toRow(d))), lastOnly = false, isSorted = false)
    }
  ...
  object CheckLastBatch {
    ...
    def apply[A: Encoder](isSorted: Boolean, data: A*): CheckAnswerRows = {
      val encoder = encoderFor[A]
      val toExternalRow = RowEncoder(encoder.schema).resolveAndBind()
      CheckAnswerRows(data.map(d => toExternalRow.fromRow(encoder.toRow(d))), lastOnly = true, isSorted = isSorted)
    }
  ...
  case class CheckNewAnswerRows(expectedAnswer: Seq[Row]) extends StreamAction with StreamMustBeRunning {
  ...
  case class SetManualClock(time: Long) extends StreamAction
   ...
   */
  case class StreamProgressLockedActions(actions: Seq[StreamAction], desc: String = null) extends StreamAction {
  ...
  class Assert(condition: => Boolean, val message: String = "") extends StreamAction {
    def run(): Unit = {
      val _ = Assertions.assert(condition)
    }
  ...
  class AssertOnQuery(val condition: StreamExecution => Boolean, val message: String) extends StreamAction {
  ...
  def testStream(
    ...
    useV2Sink: Boolean = defaultUseV2Sink
    ...
    synchronized {
      ...
      def fetchStreamAnswer(
        ...
        verify(!(lastOnly && sinceLastFetchOnly), "both lastOnly and sinceLastFetchOnly cannot be true")
        ...
        val rows =
          ...
          } else if (lastOnly) sink.latestBatchData
          else sink.allData
          catch {
            case e: Exception =>
      ...
      def executeAction(action: StreamAction): Unit = {
        ...
        action match {
          ...
          case SetManualClock(time) =>
            verify(currentStream != null, "can not advance manual clock when a stream is not running")
            verify(
          ...
          case StopStream =>
            ...
            try failAfter(streamingTimeout) {
              ...
              case e: org.scalatest.exceptions.TestFailedDueToTimeoutException =>
                failTest("Timed out while stopping and waiting for microbatchthread to terminate.", e)
              case t: Throwable =>
          ...
          case a: AssertOnQuery =>
            verify(currentStream != null || lastStream != null, "cannot assert when no stream has been started")
            val streamToAssert = Option(currentStream).getOrElse(lastStream)
            try verify(a.condition(streamToAssert), s"Assert on query failed: ${a.message}")
            catch {
              case NonFatal(e) =>
          ...
          case a: Assert =>
            val _ = Option(currentStream).getOrElse(lastStream)
          ...
          case a: AddData =>
            try {
              ...
              val sourceIndex =
                queryToUse
                  .flatMap(query => findSourceIndex(query.logicalPlan))
                  .orElse {
                    findSourceIndex(stream.logicalPlan)
                  }
                  .orElse {
                    queryToUse.flatMap(q => findSourceIndex(q.lastExecution.logical))
                  }
                  .getOrElse {
              ...
              val _ = awaiting.put(sourceIndex, offset)
            } catch {
              case NonFatal(e) =>
          ...
          case CheckAnswerRows(expectedAnswer, lastOnly, isSorted) =>
            ...
            QueryTest.sameRows(expectedAnswer, sparkAnswer, isSorted).foreach(error => failTest(error))
          ...
          case CheckAnswerRowsContains(expectedAnswer, lastOnly) =>
            ...
            QueryTest.includesRows(expectedAnswer, sparkAnswer).foreach(error => failTest(error))
          ...
          case CheckAnswerRowsByFunc(globalCheckFunction, lastOnly) =>
            ...
            try globalCheckFunction(sparkAnswer)
            catch {
          ...
          case CheckNewAnswerRows(expectedAnswer) =>
            ...
            QueryTest.sameRows(expectedAnswer, sparkAnswer).foreach(error => failTest(error))
        }
      }
   ...
   */
  def runStressTest(ds: Dataset[Int], addData: Seq[Int] => StreamAction, iterations: Int = 100): Unit =
  ...
  object AwaitTerminationTester {
    ...
    case class ExpectException[E <: Exception]()(implicit val t: ClassTag[E]) extends ExpectedBehavior
    ...
    def test(
      ...
      expectedBehavior match {
        ...
        case e: ExpectException[_] =>
          ...
          val _ = assert(thrownException.cause.getClass === e.t.runtimeClass, "exception of incorrect type was throw")
      }
  }
}
